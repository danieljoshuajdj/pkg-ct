...
describe('scanSourceUsage', () => {
  it('uses config-aware evidence to avoid framework false positives', async () => {
    ...
    expect(usage.packageUsage.get('react')?.confidence).toBe(50);
    expect(usage.packageUsage.get('react-dom')?.confidence).toBe(30);
    expect(usage.packageUsage.get('tailwindcss')?.confidence).toBe(30);
    expect(usage.packageUsage.get('@cloudflare/vite-plugin')?.confidence).toBe(45);
    expect(usage.packageUsage.get('@tanstack/router-plugin')?.confidence).toBe(45);
    expect(usage.packageUsage.get('eslint-config-prettier')?.confidence).toBe(30);
  });
});
